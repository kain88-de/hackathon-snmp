...
const dragging = ref<null | {
  kind: 'pan' | 'create' | 'resize-left' | 'resize-right';
  startX: number;
  startWinS: number;
  startWinE: number;
}>(null);
...
function drawDetail() {
  ...
  const visible = props.exchanges
    .filter((ex) => ex.sentAtMs < wE && ex.receivedAtMs > wS)
...
function onMiniMousedown(event: MouseEvent) {
  ...
  if (x >= selX - EDGE && x <= selX + EDGE) {
    ...
    dragging.value = {
      kind: 'create',
      startX: x,
      startWinS: getMiniMs(x),
      startWinE: getMiniMs(x),
    };
  }
}
...
function onMiniMousemove(event: MouseEvent) {
  ...
  switch (dragging.value.kind) {
    case 'pan': {
      ...
      const clampedDms = Math.max(
        -dragging.value.startWinS,
        Math.min(mT - dragging.value.startWinE, dms),
      );
