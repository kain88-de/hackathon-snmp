...
import type { DomainExchange, FilterState, FlatRow, TrieLeaf, TrieNode } from './model'
...
export function buildTrie(exchanges: ReadonlyArray<DomainExchange>, state: Readonly<FilterState>): TrieNode {
  const root = makeNode('', '')
...
export function rollup(node: TrieNode, slowMs: number): void {
  ...
  for (const leaf of node.leaves) {
    const leafSeverity: 0 | 1 | 2 =
      leaf.exchange.violations.length > 0 ? 2
      : leaf.exchange.rtt > slowMs ? 1
      : 0
    node.stats.count += 1
...
function visitNode(node: TrieNode, depth: number, rows: FlatRow[]): void {
  ...
  if (node.expanded) {
    ...
    for (const leaf of node.leaves) {
      rows.push({ kind: 'leaf', depth: depth + 1, exchange: leaf.exchange, shared: leaf.shared })
    }
  }
}
