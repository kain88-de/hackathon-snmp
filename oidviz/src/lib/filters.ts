...
export function matchesFilters(ex: Readonly<DomainExchange>, state: Readonly<FilterState>): boolean {
...
export function clusterMatchesFilters(incident: Readonly<Incident>, state: Readonly<FilterState>): boolean {
  const { slow, violations, retries, timeouts } = state
  if (!slow && !violations && !retries && !timeouts) return true
